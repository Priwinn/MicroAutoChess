...
namespace MicroAutoChess.AvaloniaApp
{
    public partial class MainWindow : Window
    {
        ...
        private void OnOpened(object? sender, EventArgs e)
        {
                ...
                if (_pveManager != null && _spawnSpecs != null && _skiaVisualizer != null && _engine != null)
                {
                    ...
                    for (int i = 0; i < total; i++)
                    {
                        ...
                        if (mx >= r.Left && mx <= r.Right && my >= r.Top && my <= r.Bottom)
                        {
                            ...
                            // only allow spawning before combat starts
                            if (!_paused && _engine.FrameNumber > 0) { ev.Pointer.Capture(null); return; }

                            var spec = _spawnSpecs[i];
                            var ut = spec.ut;
                            // compute cost using a temporary unit
                            var tmp = new Unit(ut, UnitRarity.COMMON, Team.TEAM_2);
                            int cost = tmp.GetCost();
                            if (_pveManager.PlayerBudget < cost) { ev.Pointer.Capture(null); return; }

                            // find bench slot first
                            int benchIdx = -1;
                            for (int bi = 0; bi < _board.BenchSize; bi++)
                            {
                                var b = _board.GetBenchUnit(Team.TEAM_2, bi);
                                if (b == null) { benchIdx = bi; break; }
                            }
                            ...
                            if (benchIdx != -1)
                            {
                                ...
                                // place in board's bench structures
                            ...
                            else
                            {
                                ...
                                // sync board from player state
                            }

                            // deduct budget via PvE manager API
            ...
            _image.PointerReleased += (s, ev) => {
                ...
                if (_skiaVisualizer.IsDragging)
                {
                    ...
                    try
                    {
                        var sold = TryPopDraggedUnitForSell(mx, my, total);
                        if (sold.Item1 != null)
                        {
                            var su = sold.Item1;
                            // refund full cost (mirror Python behavior)
                            _pveManager.AddBudget(su.GetCost()); 

                            // remove from player's mappings (bench or board)
                            if (p2 != null)
                            {
                                var toRem = p2.UnitsOnBoard.Where(kv => kv.Value == su).Select(kv => kv.Key).ToList();
                                foreach (var k in toRem) p2.UnitsOnBoard.Remove(k);
                                if (sold.Item2.HasValue)
                                {
                                    var fr = sold.Item2.Value;
                                    if (fr.Item1 == -2) p2.Bench[fr.Item2] = null;
                                }
                            }
        ...
        private (Unit?, (int, int)?) TryPopDraggedUnitForSell(int mouseX, int mouseY, int spawnTotal = 4)
        {
            if (_skiaVisualizer == null) return (null, null);
            var dragged = _skiaVisualizer.DraggedUnit;
            var dragFrom = _skiaVisualizer.DragFrom;
            if (dragged == null || !dragFrom.HasValue) return (null, null);
            if (_skiaVisualizer.IsMouseOverShop(mouseX, mouseY, spawnTotal))
            {
                _skiaVisualizer.ClearDragState();
                return (dragged, dragFrom.Value);
            }
            return (null, null);
        }
        ...
        private bool EndDragAt(int mouseX, int mouseY)
        {
            ...
            bool placed = false;
            var target = _skiaVisualizer.HitTestBoardCell(mouseX, mouseY);
            if (target.HasValue)
            {
                // Re-insert the dragged unit back to its original slot so PlayerMoveUnit can find it.
                _board.PlaceUnit(dragged, dragFrom.Value);
                placed = _board.PlayerMoveUnit(dragFrom.Value, target.Value, dragged.Team);
                if (!placed)
                {
                    if (_board.IsValidPosition(target.Value) && _board.IsEmpty(target.Value)) placed = _board.PlaceBoardUnit(dragged, target.Value);
                }
            }
            else
            {
                var benchTarget = _skiaVisualizer.HitTestBenchCell(mouseX, mouseY);
                if (benchTarget.HasValue)
                {
                    // Re-insert dragged unit to origin so PlayerMoveUnit can reference it (needed for bench->bench).
                    _board.PlaceUnit(dragged, dragFrom.Value);
                    placed = _board.PlayerMoveUnit(dragFrom.Value, benchTarget.Value, dragged.Team);
                }
            }
