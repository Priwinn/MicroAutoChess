...
def main():
    ...
    visual = PygameBoardVisualizer(board, render_fps=render_fps, cell_radius=32)
    ...
    try:
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if paused and engine.frame_number == 0:
                        ...
                        if bench_cell is not None:
                            ...
                            if bteam == -2:
                                bench_unit = board.get_bench_unit(-bteam, bindex)
                                if bench_unit is not None:
                                    dragged_unit = board.remove_bench_unit(-bteam, bindex)
                                    dragging = True
                                    drag_from = (bteam, bindex)
                                    visual.highlight_player_initial_zone = True
                                    continue
                    ...
                    if dragging:
                        try:
                            ...
                            if shop_rect.collidepoint(event.pos) and dragged_unit is not None:
                            ...
                            if not sold:
                                ...
                                if target is not None and board.is_valid_position(target):
                                    ...
                                    #         if tcell.is_empty():
                                    #             board.place_board_unit(dragged_unit, target)
                                    #             placed = True
                                    #         else:
                                    ...
                                    #                     board.place_board_unit(other_unit, drag_from)
                                    #                     placed = True
                                    #                     # engine.update_players(player1, player2)
            ...
            if 'dragging' in locals() and dragging and dragged_unit is not None:
                ...
                # draw sell hint above shop area
                spawn0 = visual.get_spawn_button_rect(0, total=5)
                pause_r = visual.get_pause_button_rect()
                # align shop left with board left offset so it sits directly below the board
                shop_x = visual.left_offset + visual.margin
            ...
            # show current simulation FPS (engine_fps) above the x2 button
            fps_text = visual.tooltip_font.render(f"Engine FPS: {engine_fps}", True, (240, 240, 160))
            tx = pause_r.centerx - fps_text.get_width() // 2
            ty = speed_up_rect.y - fps_text.get_height() - 4
            ...
            if (not team1_alive or not team2_alive) and engine.frame_number > 0:
                ...
                if not team1_alive:
                    ...
                    if advanced:
                        ...
                        player1, player2 = pve_manager.apply_round_to_board(board, player1, player2)
                        engine = CombatEngine(board, player1, player2, combat_seed=42)
                        ...
                        # no more rounds -> show magnificent win screen then reset to first round
